...
#include "../include/Send.hpp"
using namespace NSend;
inline string & SendHttp::getUrl(){
    return http->url;
}
inline const map<string,string> & SendHttp::getMap(){
    return http->header;
}
...
{
    char * send_html,file_type[NUMS::MAX_BUFF],buf[NUMS::MAX_BUFF];
